...
- **Source event on show/hide/remove events** — `KompElement.remove(eventOptions)` now forwards a CustomEvent init dict to the `remove` and `removed` events it fires, and `Floater.show(eventOptions)` / `hide(eventOptions)` / `toggle(shouldHide, eventOptions)` / `hideNow(eventOptions)` do the same for `show`, `shown`, `hide`, and `hidden`. Components that show or dismiss themselves in response to user input pass the originating event along as `event.detail.sourceEvent`, so listeners can tell an Escape keypress from an outside click from a close-button click, or a dropdown opened by click from one opened by ArrowDown. Covered: `Floater` (Escape, blur), `Dropdown` (anchor click/Enter/Space), `Tooltip` (delegated mouseover/focusin/mouseleave/blur/Escape), `Select` (ArrowUp/ArrowDown, typeahead), `Modal` (Escape), `Notification` (close button), and `Resizer` (autohide mouseleave). `Modal.remove`, `Tooltip.remove`, and `Dropdown.show`/`hide` forward their arguments to `super`. The previous `remove(callback)` signature is deprecated: passing a function still awaits it between `remove` and `removed`, but logs a warning.
- **`Dropzone#fileDrop` source event** — `event.detail` gains `sourceEvent`, the `drop` event, whose `dataTransfer` carries the parts of the drop the `file` alone doesn't: sibling items, drop coordinates, and modifier keys. One drop fires one `fileDrop` per file, all sharing the same `sourceEvent`.
- **`Floater.holdOpen()` / `releaseHoldOpen()`** — Suppresses `removeOnBlur` dismissal while set, via a `hold-open` attribute. Use it around interactions that move focus out of the floater but shouldn't close it (a native file picker, a nested overlay). `checkFocus` additionally ignores blur events when the floater still contains the active element.
- **Multi-MIME copy/paste for `Spreadsheet`** — `DataGridColumn#valueFor` may now return `{ 'text/plain', 'text/html' }` instead of a bare string, to carry a richer representation for columns whose stored value is opaque (e.g. a belongs-to id). `Spreadsheet#copyCells` writes both representations in a single `ClipboardItem` via `navigator.clipboard.write` (the `text/html` payload is one `<table>` for the selection), falling back to `writeText` when the async write API is unavailable. On paste, `Spreadsheet` reads both representations from the clipboard (or the native `paste` event's `clipboardData`) and, when the pasted `text/html` table lines up with the plain-text/TSV matrix, hands each column whichever it declared via the (now ordered, most-preferred-first) `SpreadsheetColumn.pasteAccepts` — `paste(record, value, type)` and `parsePaste(value, type)` receive the chosen MIME type so a column can pull its own structure (e.g. a `data-id` attribute) back out of pasted HTML. A bare string `valueFor` or a mismatched/missing HTML payload falls back to the existing `text/plain` behavior unchanged (#82).
- **`DataGridColumn#renderAttribute`** — `renderContent`'s no-`render`-configured fallback (read `attribute` off the record) is now its own method, `renderAttribute(record)`. A column type overrides `renderAttribute` instead of `renderContent` to format that value (e.g. a date column formatting a Date), and a caller-supplied `render` function can call `column.renderAttribute(record)` to get that type's default formatting for a value it computed itself, rather than reimplementing it.

### Bug Fixes

- **Floater Escape on keydown** — `Floater.checkEscape` now listens on `keydown` rather than `keyup`, and tests `e.key !== 'Escape'` instead of the deprecated `e.which != 27`. Every other Escape handler in the library (`Modal`, `Spreadsheet`, `Tooltip`, `Mosaic`) is already on `keydown`, so a Dropdown inside a Modal saw the modal close on keydown and the floater close on the keyup of the same press.
- **`Floater.show` re-show argument leak** — When a show was requested while a removal was still pending, `show` queued `this._removing.then(this.show)`. The promise resolves with the element, so the re-show received the floater itself as its first argument. Harmless while `show` took no parameters; now that it takes `eventOptions`, the element would have been handed to `trigger()` as the CustomEvent init dict. Now queues an arrow function.
- **`Dropdown.anchorChanged` listener cleanup** — The teardown branch called `now.removeEventListener('keydown', …)` inside the `was` block, so the old anchor kept its keydown listener and a null `now` threw. Now removes from `was`.
...
- **DataGrid cells render their content in a block; `collapseTo` is now a content height** — `DataGridCell` renders into `DataGridCell#contentElement`, a `komp-data-grid-cell-content` (`komp-spreadsheet-cell-content`) element created once per pooled cell. It is `display: contents` — no box, no effect on layout — until `collapsible` gives it one, so a cell renders exactly as it did before, including cells an app formats as a flex or grid container.

  `collapsible` bounds and clamps that block rather than the cell, because capping the cell detached it from its row's track: a truncated cell stopped at the budget while its neighbours stretched to the track, so its border-bottom no longer met the row's edge (visibly stepped grid lines) and the `Spreadsheet` selection box, sized from the row geometry, outlined a taller box than the cell it belonged to. Bounding a block inside the cell leaves the cell filling its track. It also retires the sliver of the next line that used to show under the ellipsis: the clamp doesn't shorten its box (the lines past the ellipsis are still laid out), so the block is pinned to `lines × line-height` — and unlike the cell it has no padding for a clipped line to paint into, since `overflow: hidden` clips at the padding box. The row-budget pull-down and its second measure pass are gone with it.

  The budget is read per cell, so an app can exempt a column by setting `--collapseTo: none` on it — worth doing for cells holding an avatar or an icon button, which are taller than a line of text and would otherwise read as overflow and offer to expand content that was never cut off. Cells with a budget carry a `budgeted` attribute (`clamped` still marks the truncated ones).

  Migration: `collapseTo` excludes the cell's padding and borders, so subtract them from any px budget, or write it in lines (`collapseTo: '3lh'`); anything selecting cell children (`komp-spreadsheet-cell > *`, `:first-child`) now sees the content block; `clampCell` no longer returns a height, and `clampRowCells` returns just the overflowing pairs.
