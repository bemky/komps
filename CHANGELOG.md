...
- **Source event on dismissal events** — `KompElement.remove(eventOptions)` now forwards a CustomEvent init dict to the `remove` and `removed` events it fires, and `Floater.hide(eventOptions)` does the same for `hide` and `hidden`. The components that dismiss themselves in response to user input pass the originating event along as `event.detail.sourceEvent`, so listeners can tell an Escape keypress from an outside click from a close-button click: `Floater` (Escape and blur), `Modal` (Escape), `Notification` (close button), and `Resizer` (autohide mouseleave). `Modal.remove` and `Tooltip.remove` forward their arguments to `super`. The previous `remove(callback)` signature is deprecated: passing a function still awaits it between `remove` and `removed`, but logs a warning.
