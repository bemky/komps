/**
 * Column controller for a {@link Spreadsheet} — extends {@link DataGridColumn}
...
export default class SpreadsheetColumn extends DataGridColumn {
     ...
     * @param {SpreadsheetColumn} column
     * @returns {HTMLElement}
     */
     ...
     * @param {string} value
     * @param {string} [type='text/plain'] - the MIME type `value` was pasted as (see {@link SpreadsheetColumn#pasteAccepts}); a column that accepts more than one type can branch on it (e.g. pull a `data-id` back out of pasted `text/html`).
     */
    parsePaste(value, type = 'text/plain') { return value }

    /**
     * Apply a pasted value to the record.
     * @param {Object} record
     * @param {string} value
     * @param {string} [type='text/plain'] - which MIME representation `value` is, chosen by {@link Spreadsheet#pasteMatrixAt} from the column's `pasteAccepts` — forwarded to `parsePaste`.
     */
    paste(record, value, type = 'text/plain') {
        if (this.attribute != null) record[this.attribute] = this.parsePaste(value, type)
    }
    ...
    /**
     * MIME types this column accepts on paste, most preferred first — a column that lists
     * `'text/html'` before `'text/plain'` is handed the pasted HTML (e.g. a `<td>`'s
     * innerHTML) when the clipboard carried a matching HTML table, falling back to
     * `'text/plain'` otherwise (see {@link Spreadsheet#pasteMatrixAt}).
     */
