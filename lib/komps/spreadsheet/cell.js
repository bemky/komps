...
export default class SpreadsheetCell extends TableCell {
    ...
    onFocusIn (e) {
        this.focusCell = createElement(this.table.tagName + "-focus", {
            class: Array.from(this.classList.values().filter(x => x.includes("frozen"))).join(" ")
        })
        this.focusCell.style.setProperty('grid-area', `1 / ${this.cellIndex} / -1`)
        if (this.cellSplitIndex) {
            this.focusCell.style.setProperty('grid-row', this.cellSplitIndex)
        }
        this.row.append(this.focusCell)
