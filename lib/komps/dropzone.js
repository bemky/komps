 ...
 * @param {function} [options.onFileDrop] - event listener for fileDrop, receives `{ file, sourceEvent }` in event detail
...
/**
 * Fired once per file dropped onto the dropzone. `event.detail.file` is the `File`;
 * `event.detail.sourceEvent` is the `drop` event, whose `dataTransfer` carries the
 * full drop (sibling items, coordinates, modifier keys). The same `sourceEvent` is
 * shared by every file in one drop.
...
export default class Dropzone extends KompElement {
    ...
    connected () {
        ...
        if (!this.hasAttribute('aria-label')) {
            this.setAttribute('aria-label', 'File drop zone');
        }
    ...
    drop (e) {
        if (e.target == this || this.contains(e.target)) {
            ...
            ([...e.dataTransfer.files]).forEach(file => {
                this.trigger('fileDrop', { detail: { file, sourceEvent: e } })
            });
        }
