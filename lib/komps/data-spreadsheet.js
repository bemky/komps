...
export default class DataSpreadsheet extends DataGrid {
    ...
    constructor(...args) {
        ...
        this._copyRange = null       // snapshot of the copied range, for the "marching ants" marquee
    }
    ...
    paintSelection() {
        ...
        this.renderCopyIndicator()
    }

    /** The solid border box around the live selection range. */
    renderSelectionIndicator(r = this.range()) {
        this._selectIndicator = this.positionIndicator(this._selectIndicator, r)
    }

    /** The dashed "marching ants" box around the copied range (snapshot). */
    renderCopyIndicator() {
        this._copyIndicator = this.positionIndicator(this._copyIndicator, this._copyRange, 'copy')
    }

    /** Cancel the copy marquee. */
    clearCopyIndicator() {
    ...
    /**
     * Position (creating/removing as needed) a body-space `select-indicator` box for a
     * range. Geometry comes from row/column geometry (not the cells), so it spans
     * off-screen rows/columns; lives in body space (scrolls with content, hidden under
     * the sticky header). Returns the element, or null when the range is empty.
     *
     * Known limitation (as in Spreadsheet): body-space positioning means a range
     * starting in a frozen column won't track that column when scrolled sideways.
     */
    positionIndicator(el, r, cls) {
        if (!r) {
            if (el) el.remove()
            return null
        }
        if (!el) {
            el = createElement(`${this.localName}-select-indicator`, cls ? { class: cls } : {})
            this.body.append(el)
        }
    ...
    moveActive(dir, extend = false) {
        if (extend) {
            ...
            this.clearCopyIndicator() // moving to another cell cancels the copy marquee
    ...
    copyCells(e) {
        ...
        // Mark the copied range with the dashed marquee until paste/Escape/new activation.
        this._copyRange = this.range()
        this.renderCopyIndicator()
    }
    ...
    setupSelection() {
        ...
        this.addEventListenerFor(sel, 'mousedown', e => {
            ...
            if (e.shiftKey && this.active) {
                ...
                this.clearCopyIndicator() // activating another cell cancels the copy marquee
            }
        ...
        this.addEventListenerFor(sel, 'keydown', e => {
            ...
            if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(key)) {
                ...
                if (this._copyRange) this.clearCopyIndicator()
