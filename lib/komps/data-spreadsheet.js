...
export default class DataSpreadsheet extends DataGrid {
    ...
    moveActive(dir, extend = false) {
        // When extending, move the range's focus edge; otherwise move the active cell.
        // The active cell (the anchor) stays put while extending.
        const cur = extend
            ? ((this.selection && this.selection.focus) || this.active)
            : (this.active || (this.selection && this.selection.focus))
        if (!cur) return
        ...
        if (extend) {
            const anchor = this.active || this._anchor || cur
            this._anchor = anchor
            this.setSelection(anchor, { row, col }) // active unchanged
        } else {
            this.active = { row, col }
            this._anchor = { row, col }
            this.setSelection({ row, col })
        }
        this.focusCellAt(row, col) // focus/scroll follow the moving point (edge when extending)
    }
    ...
    setupSelection() {
        ...
        this.addEventListenerFor(sel, 'mousedown', e => {
            ...
            if (e.shiftKey && this.active) {
                // Shift-click extends the range from the existing anchor; active stays.
                this.setSelection(this._anchor || this.active, pos)
            } else {
                this._anchor = pos
                this.active = pos
            ...
            // Drag extends the range from the anchor; the active cell stays put.
            const move = ev => {
                const c = ev.target instanceof Element ? ev.target.closest(sel) : null
                if (c && c.column) this.setSelection(this._anchor, { row: c.rowIndex, col: c.cellIndex })
            }
    ...
    static style = function () { return `
        ...
        /* Grid-level overlay layer for transient UI (edit floater, future resize
           indicators). Zero-size so it blocks nothing; a high z-index lifts its
           children above the sticky header (30) / frozen header-cell (35); the grid's
           overflow:auto still clips them, so editors overflow the cell, not the grid. */
        ...
        /* No :focus outline — the active cell (anchor) is the marker; while extending,
           DOM focus follows the moving edge and shouldn't look active. */
        ${this.tagName}-cell:focus { outline: none; }
