...
export default class DataSpreadsheet extends DataGrid {
    ...
    static events = [...DataGrid.events, 'invalidPaste', 'cellChange', 'cellChanged']
    ...
    constructor(...args) {
        ...
        this.cellSelector = this.constructor.Cell.tagName
    }
    ...
    focusCellAt(row, col) {
        ...
        this.updateWindow() // mount synchronously so the target cell exists
        const cell = this.cellElementAt(row, col)
    ...
    activateCell(cell, options = {}) {
        ...
        const input = column.editInput(record)
        if (!input) return
        input.classList.add(`${this.localName}-editor`)
        if (options.value != null) input.value = options.value

        cell.appendChild(input)
        this._editing = { cell, column, record, input }
        input.focus()
        if (input.select && input.tagName === 'INPUT') input.select()

        input.addEventListener('keydown', e => {
            e.stopPropagation()
            if (e.key === 'Enter') { e.preventDefault(); this.commitEdit(); this.moveActive('down') }
            else if (e.key === 'Escape') { e.preventDefault(); this.cancelEdit() }
            else if (e.key === 'Tab') { e.preventDefault(); this.commitEdit(); this.moveActive(e.shiftKey ? 'left' : 'right') }
        })
        if (input.tagName === 'SELECT') input.addEventListener('change', () => this.commitEdit())
        input.addEventListener('blur', () => { if (this._editing && this._editing.input === input) this.commitEdit() }, { once: true })
    }
    ...
    commitEdit() {
        ...
        if (!ed) return
        this._editing = null
        ed.column.write(ed.record, ed.column.readInput(ed.input))
        ed.cell.render()
        ed.cell.focus({ preventScroll: true })
        this.trigger('cellChanged', { detail: { cell: ed.cell, column: ed.column, record: ed.record } })
    }
    cancelEdit() {
        const ed = this._editing
        if (!ed) return
    ...
    copyText() {
        ...
        for (let row = r.r0; row <= r.r1; row++) {
            ...
            const cols = []
            for (let col = r.c0; col <= r.c1; col++) cols.push(this.quote(this.columns[col].serialize(record)))
    ...
    static style = function () { return `
        ...
        ${this.tagName}-editor {
            ...
            box-sizing: border-box; border: none; outline: 2px solid var(--ds-select, #1a73e8);
            font: inherit; padding: inherit; background: white; z-index: 4;
        }
    `}
