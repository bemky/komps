...
export default class DataSpreadsheet extends DataGrid {
     ...
     * while a copy is pending (the marquee). Geometry comes from row/column geometry
     * (not the cells), so it spans off-screen rows/columns; lives in body space
     * (scrolls with content, hidden under the sticky header).
     *
    ...
    pasteData(text) {
        ...
        this.setSelection(start, { row: endR, col: endC }) // also clears the copy marquee
