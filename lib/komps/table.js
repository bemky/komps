    ...
    async #spliceColumns(index, countToDelete, newColumns=[], mutate=true) {
        newColumns = await Promise.all(newColumns.map(async (column, i) => {
            if (column instanceof TableColumn) {
                ...
                column.cells.forEach(c => c.remove())
                return column
            }
        ...
        if (newColumns.length > 0) {
            ...
            await Promise.all(await (await this.data).map(async (record, rowIndex) => {
                ...
                return Promise.all(newColumns.map(async column => {
                    let cell = Array.from(column.cells).find(c => c.record == record)
                    cell = cell || await this.renderColumnCell(column, record)
                    const anchor = row.children.item(index - 1)
                    if (anchor) {
                        await insertBefore(anchor, cell)
                    } else {
                        await insertAfter(row.children, cell)
                    }
                }))
            }))
        }
                ...
                return row.cells.map((cell, index, cells) => {
                    ...
                    if (cellFromIndex <= cell.cellIndex && cell.cellIndex <= cellToIndex) {
                        if (cell.isSplit) {
                            ...
                            const toSplitCellIndex = to[2] == undefined ? cell.cells.length : to[2]
                            return Array.from(cell.cells).filter(splitCell => {
