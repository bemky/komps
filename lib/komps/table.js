...
export default class Table extends KompElement {
    ...
    connected () {
        if (!this.constructor.rowMutationObserver) {
            this.constructor.rowMutationObserver = new MutationObserver(mutations => {
        ...
        this.constructor.rowMutationObserver.observe(this, {childList: true})
    }
    ...
    renderFrozenDivider () {
        ...
        if (frozenColumns.length > 0) {
            let indicator = this.querySelector(`${this.localName}-frozen-indicator.column`)
            if (!indicator) {
                indicator = this.observeIntersection(createElement(`${this.localName}-frozen-indicator`, {
                    class: 'column',
                    data: {
            ...
            let divider = this.querySelector(`${this.localName}-frozen-divider.column`)
            if (!divider) {
                divider = createElement(`${this.localName}-frozen-divider`, {
                    class: 'column'
                })
                append(this, divider)
            }
            divider.style.setProperty('grid-column', `1 / ${frozenColumns.length + 1}`)
        } else {
            this.querySelector(`${this.localName}-frozen-indicator.column`)?.remove()
            this.querySelector(`${this.localName}-frozen-divider.column`)?.remove()
        }
    }
    ...
    renderHeader () {
        ...
        return [
            this.header,
            this.observeIntersection(createElement(`${this.localName}-frozen-indicator`, {
                class: 'row',
                data: {
                    target: `${this.localName}-frozen-divider.row`
                }
            })),
            createElement(`${this.localName}-frozen-divider`, {
                class: 'row',
                style: {
                    'grid-row': '1'
                }
