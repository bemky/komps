...
export default function (proto) {
    ...
    this.columnTypeRegistry.default.assignableAttributes.push('resize')
    
    const renderHeaderWas = proto.renderHeader
    proto.renderHeader = function (...args) {
        const result = renderHeaderWas.call(this, ...args)
        Array.from(result.children).forEach((cell, index) => {
            const column = this.columns[index]
            if (this.resize.includes('columns') && column.resize != false) {
                cell.classList.add('resizable') // remove?
                cell.classList.toggle('resizable-column', true)
            }
        })
        return result
    }
    ...
    proto.activateAxisResize = function (e) {
        ...
        let { direction, axis, offset, slice, start, blockDimension, inlineDimension, index } = handle.classList.contains('resizable-row') ? {
            direction: 'Row',
            axis: 'y',
        ...
        const slices = uniq(Array.from(this.querySelectorAll(`${this.constructor.Header.tagName} > .selected`)).map(cell => {
            return direction == 'Column' ? cell.column : cell.row
        }))
        if (slices.length == 0) {
            slices.push(slice)
        }
        ...
        window.addEventListener('mouseup', e => {
            ...
            slices.forEach(slice => {
                slice[inlineDimension] = delta + "px"
            })
