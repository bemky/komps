/**
 * A plugin to make {@link Table}'s columns and rows resizable
 *
 * @function resizable
 * @mixin
 *
 ...
 * |---|---|---|
 * | `columnResize` | `changes:Array` | After a column resize finishes |
 * | `rowResize` | `changes:Array` | After a row resize finishes |
 *
...
export default function (proto) {
    ...
    proto.activateAxisResize = function (e) {
        ...
        let {
          direction, axis, axisMin, axisMax, offset, slice,
          start, blockDimension, inlineDimension, inlinePosition,
          index, sliceType, sliceIndex } = handle.classList.contains('resizable-row') ? {
            direction: 'Row',
            axis: 'y',
        ...
        this.addEventListener('pointerup', e => {
            ...
            const startingIndex = Math.min(...slices.map(s => s[sliceIndex]))
            // To avoid preceding slices changing shape when altering selected slice, we set them
            // this happened when columns were filling the remaining width
            this[sliceType].filter(x => x[sliceIndex] < startingIndex).forEach(slice => {
              slice[inlineDimension.toLowerCase()] = slice.cells.values().toArray().at(0)['offset'+inlineDimension] + "px"
            })
