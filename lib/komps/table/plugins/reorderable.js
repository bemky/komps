...
export default function (proto) {
    ...
    proto.activateAxisReorder = function (e) {
        ...
        const placementIndicator = createElement(`${this.constructor.tagName}-placement-indicator`, {
            ...
            style: {
                [`grid-${direction.toLowerCase()}`]: handle.cell[index]
            }
        })
        ...
        const pointerMove = e => {
            ...
            if (cell) {
                const newIndex = cell[index] + (cell[index] >= handle.cell[index] ? 1 : 0)
                if (placementIndicator[index] != newIndex) {
                    if (scrolling) return
                ...
                placementIndicator[index] = newIndex
                placementIndicator.style[`grid-${direction.toLowerCase()}`] = newIndex
            }
        }
        ...
        this.addEventListener('pointerup', async e => {
            ...
            if (placementIndicator[index] != indexStart) {
                let indexNew = placementIndicator[index] + (indexStart < placementIndicator[index] ? -1 : 0)
