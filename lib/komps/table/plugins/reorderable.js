    ...
    proto.activateAxisReorder = function (e) {
        ...
        const pointerMove = e => {
            ...
            if (cell) {
                ...
                if (dragIndicator[`offset${inlinePosition}`] != cell[`offset${inlinePosition}`]) {
                    if (scrolling) return
                    let start = this[`scroll${inlinePosition}`]
                    if (!handle.cell.frozen && this.frozenLeft) {
                        start = start + this.frozenLeft
                    }
                    start = start + this[`client${inlineDimension}`] * 0.1
                    const end = this[`scroll${inlinePosition}`] + this[`client${inlineDimension}`] * 0.9
