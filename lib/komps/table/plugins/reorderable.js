...
export default function (proto) {
    ...
    this.columnTypeRegistry.default.assignableAttributes.push('reorder')
    ...
    const renderRowWas = proto.renderRow
    proto.renderRow = function (...args) {
        const result = renderRowWas.call(this, ...args).then(row => {
            if (this.reorder.includes('rows')) {
                row.children.item(0).classList.add('reorderable', 'reorderable-row')
            }
            return row
        })
        return result
    ...
    proto.activateAxisReorder = function (e) {
        ...
        this.queryCells(`.${direction.toLowerCase()}-${handle.cell[index]}`).forEach(el => el.classList.add('selecting'));
