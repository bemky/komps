 ...
 * `serialize`/`parse`/`write`/`clear` take a record, and `editInput` takes a record.
...
export default class DataSpreadsheetColumn extends DataGridColumn {
    ...
    /** Display content for a record (defaults to the attribute value). */
    renderContent(record, cell) {
        ...
        if (this.attribute != null) {
            const v = record == null ? null : record[this.attribute]
            return v == null ? '' : v
        }
        return ''
    }

    /** The raw stored value for a record (what editing starts from). */
    valueOf(record) {
        return this.attribute != null && record != null ? record[this.attribute] : null
    }

    /** Serialize a record's value for copy (always a string). */
    serialize(record) {
        if (isFunction(this.value)) return String(this.value(record, this) ?? '')
        const v = this.valueOf(record)
