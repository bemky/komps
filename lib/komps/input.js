...
export default class Input {
     ...
     *
     * @param {Event} [e]
     * @param {*} [v] Value to load instead of reading the target.
     */
    _load (e, v) {
    ...
    /**
     * Empty the input. Called by {@link Input#_load} when the loaded value is
...
class SelectInput extends Input {
    createInput (options={}) {
        ...
        if (options.includeBlank) {
            this.blankOption = createElement('option', Object.assign({
                content: 'Unset',
                value: null
            }, options.includeBlank))
            input.append(this.blankOption)
        }
        if (options.options) {
    ...
    // Select the blank option rather than emptying `value`: that option's value is
    // "null" (see createInput), which `value = ''` wouldn't match, leaving the
    // select with nothing selected at all.
    clear () {
        if (this.input.multiple) {
            Array.from(this.input.options).forEach(option => option.selected = false)
