/**
 * A {@link Table} with editable cells
 *
 * @class Spreadsheet
 * @extends Table
 *
 ...
 * })
 *
 * @todo collapse selectedCells when pasting to match dimensions of copy (like google sheets)
 * @todo expand selectedCells when pasting to match dimensions of copy (like google sheets)
 * @todo select multiple columns reorder, only moves one
 * @todo edit header needs to fire event, or provide object to change
 * @todo Tab to a cell causes sheet to scroll instead of using single cell scroll of focusAdjacentCell
 * @todo insert/remove column
 * @todo copy/paste split row
 * @todo mouse select cells, then arrow select, arrow selection starts at original start of mouse and not end
 * @todo selection outline overlays and doesn't track with frozen cells
 * @todo implement ClipboardAPI / ClipboardItem for copy/paste
 */

import { createElement, listenerElement, content, insertAfter } from 'dolla';
import { result, except, placeCaretAtEnd, closest, groupBy, isInView, splitByUnquotedChar, uniq } from '../support.js';
import Table from './table.js';
import Floater from './floater.js';
import Modal from './modal.js';
import Cell from './spreadsheet/cell.js';
import SpreadsheetHeaderCell from './spreadsheet/header-cell.js';
import Column from './spreadsheet/column.js';
...
import { reorderable, resizable } from './plugins.js';
...
export default class Spreadsheet extends Table {
    ...
    static style = `
        ...
        komp-spreadsheet komp-table-header > *.selected {
            box-shadow: inset 0 2px 0 0 var(--select-color), inset 0 0 0 999px oklch(var(--select-oklch) / 0.1);
        ...
        .komp-spreadsheet-input > label > label {
            background: none;
            width: auto;
            min-height: 100%;
            min-width: 100%;
