...
```javascript
{
    ...
    input: player => createElement('input', {type: 'number', value: player.height, step: 1}),
...
type:
    types: String
    description: initialize column with predefined types (text contentarea number select checkbox radio date)
    default: 'contentarea'
...
TODO
----
- [ ] Break into separate modules ./spreadsheet/resizing.js...
...
export default class Spreadsheet extends DataTable {
    ...
    initColumn (key, options, ...args) {
        ...
        model.type = options.type || "contentarea"
        model.input = options.input == undefined ? this.defaultInput(key, options) : options.input
    ...
    defaultRender (key, options) {
        if (options.render) { return options.render }
        return {
            text: r[key].replaceAll("\n", "<br>")
        }
    ...
    defaultHeader (key, options) {
        ...
        if (typeof headerOptions != "object" && headerOptions !== undefined) {
            headerOptions = {
    ...
    renderHeader (column, ...args) {
        ...
        cell.classList.add('header', 'frozen-row')
        cell.classList.toggle('frozen-col', column.frozen === true)
        cell.tabIndex = 0;
    ...
    async activateCellInput (cell, e) {
        this.deselectCells({copySelection: true});
        let input
        if (cell.classList.contains('header')) {
            input = await result(cell.columnModel.header, 'input', cell.columnModel, cell, e)
        } else {
            input = await result(cell.columnModel, 'input', cell.record, cell, e)
        }
        if (input) {
            const rowArea = cell.classList.contains('header') ? '' : ' body'
            const colArea = ' body'
            const inputCell = createElement(this.inputTag, {
                content: {
    ...
    renderContextMenu(cell) {
        ...
        if (cell.columnModel.menu) {
            content = cell.columnModel.menu(content, cell.record)
        }
        if (this.menu) {
            content = this.menu(content, cell)
        }
    ...
    */
    ...
    static style = function() { return `
        ...
        ${this.tagName}-cell.selected {
            box-shadow: inset 0 0 0 999px rgba(26, 115, 232, 0.1), 0 0 0 1px rgba(var(--border-color), var(--border-opacity));
            &:focus {
        ...
        ${this.tagName}-input komp-content-area {
            min-height: 100%;
            min-width: 100%;
        ...
        ${this.tagName}-input.frozen-row { z-index: 41; }
        ...
        ${this.tagName}-input.frozen-col { z-index: 51; }
        ...
        ${this.tagName}-cell:focus {z-index: 61; }
...
window.customElements.define(Spreadsheet.localName, Spreadsheet);