...
- [x] Support resizing multiple rows/cols
- [x] Default row height to 1lh (need to include padding as an option for spreadsheet, because it's needed for calc), maybe read from css var
- [x] changing cell needs to scroll into view in upper direction
- [x] keyboard arrow + shift, should select

BUGS
----
- [x] Dragging column outside container does weird things to reorder-handle shadow
- [ ] Dragging column over frozen column does not scroll panel
- [x] Selecting frozen cells renderes selection box underneath frozen cells.
    Needs to be under for when selection box is for non-frozen cells
- [x] Handles render over frozen cells
- [x] Context menu is behind frozen cells
- [x] Right click paste no worky in firefox
- [x] Copy action not deselecting previous copies
- [x] Resizing Header row errors
- [x] Moving sized column needs to update grid template or else size doesn't come with it
- [x] New Lines for Default Input
- [ ] Grab reorder handle, and scroll opposite direction, and column shadow will cut off with scroll
- [x] Header Input is goofey
