...
----
resizeRow:
    arguments: height:String, index:Integer
resizeColumn:
    arguments: width:String, index:Integer
moveColumn:
    arguments: oldIndex:Integer, newIndex:Integer
moveRow:
    arguments: oldIndex:Integer, newIndex:Integer
changeColumnHeader:
    arguments: newContent:String, column:Object
...
BUGS
----
[ ] Dragging column outside container does weird things to move-handle shadow
...
export default class Spreadsheet extends DataTable {
    ...
    defaultHeader (key, options) {
        ...
        return Object.assign({
            ...
            input: options => listenerElement('input', {
                ...
                this.trigger('changeColumnHeader', e.target.value, options)
            })
        }, headerOptions)
    }
    ...
    initializeAxisResize (e) {
        ...
        let dragEnd = (e) => {
            ...
            this.trigger(`resize${axis == "col" ? "Column" : "Row"}`, newSize, startIndex)
    ...
    initializeAxisMove (e) {
        ...
        let drop = e => {
            ...
            if (cell) {
                ...
                this.trigger(`move${axis == "row" ? "Row" : "Column"}`, oldIndex - 1, newIndex - 1)
            }
    ...
    showMoveHandleFor (cell) {
        if (!this.colMoveHandle) {
            this.colMoveHandle = createElement(this.moveHandleTag, {
                class: 'col',
                draggable: true,
                content: {
                    content: handleIcon()
        ...
        if (!this.rowMoveHandle) {
            this.rowMoveHandle = createElement(this.moveHandleTag, {
                class: 'row',
                draggable: true,
                content: {
        ...
        if (!this.classList.contains('moving')) {
            this.colMoveHandle.rowIndex = 1
            this.colMoveHandle.colIndex = cell.colIndex
            this.colMoveHandle.style.gridArea = ['handles', `body ${cell.colIndex}`, 'handles', `body ${cell.colIndex}`].join(" / ")
            if (cell.classList.contains('frozen-col')) {
                this.colMoveHandle.remove();
            } else {
                this.append(this.colMoveHandle);
            }

            this.rowMoveHandle.rowIndex = cell.rowIndex
            this.rowMoveHandle.colIndex = 1
            this.rowMoveHandle.style.gridArea = [`body ${cell.rowIndex}`, 'handles', `body ${cell.rowIndex}`, 'handles'].join(" / ")
            if (cell.classList.contains('frozen-row')) {
                this.rowMoveHandle.remove();
            } else {
                this.append(this.rowMoveHandle);
            }
        }
    }
