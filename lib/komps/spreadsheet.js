...
- [ ] Different Input types (checkbox, number, date, select)
- [ ] Support Escape out of edit
...
import { indexOfNode, closest, result, groupBy, placeCaretAtEnd } from '../support.js';
...
export default class Spreadsheet extends DataTable {
    ...
    defaultInput (key) {
        return (r, cell) => {
            ...
            const input = listenerElement('div', {
                ...
                r[key] = e.target.innerText.replaceAll("<br>", "\n")
            })
            return input
        }
    }
    ...
    activateCellInput (cell) {
        ...
        if (cell.classList.contains('header')) {
            input = result(cell.columnModel.header, 'input', cell.columnModel, cell)
        } else {
            input = result(cell.columnModel, 'input', cell.record, cell)
        }
        if (input) {
            const rowArea = cell.classList.contains('header') ? '' : ' body'
            const colArea = ' body'
            const inputField = createElement(`${this.tagName}-inputcell`, {
                content: {
            ...
            const firstInput = inputField.querySelector('input, textarea, button, [contenteditable]')
            if (firstInput) {
    ...
    static style = function() { return `
        ...
        ${this.tagName}-inputcell.frozen-col,
        ${this.tagName}-inputcell.frozen-row {
            position: sticky;
            left: 0;
            top: auto;
            z-index: 41;
        ...
        ${this.tagName}-inputcell.frozen-row.frozen-col {
            z-index: 43;
        }
        ...
        ${this.tagName}-inputcell .${this.tagName}-input {
            min-width: 100%;
            outline: none;
            border: none;
            background: none;
