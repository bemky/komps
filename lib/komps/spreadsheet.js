 ...
 *
 * @fires Spreadsheet#cellChanged
 * @fires Spreadsheet#invalidPaste
 */
...
import SpreadsheetColumn from './spreadsheet/column.js'
import NumberColumn from './spreadsheet/columns/number-column.js'
import SelectColumn from './spreadsheet/columns/select-column.js'
import CheckboxColumn from './spreadsheet/columns/checkbox-column.js'
import ReadonlyColumn from './spreadsheet/columns/readonly-column.js'

export default class Spreadsheet extends DataGrid {
    static tagName = 'komp-spreadsheet'

    static Cell = SpreadsheetCell
    static Column = SpreadsheetColumn
    static columnTypeRegistry = {
        default: SpreadsheetColumn,
        number: NumberColumn,
        select: SelectColumn,
    ...
    setupSelection() {
        ...
        // Listen on the root node (document, or the shadow root hosting this grid), not `this`.
        // Some browsers (e.g. Firefox) dispatch `copy`/`paste` at the node containing the
        // selection anchor, falling back to the document body when there's no selection —
        // never at the grid itself — so a listener on `this` can silently never fire there.
        ...
        this.cleanupEventListenerFor(this.getRootNode(), 'paste', e => {
            if (this.selection && !this._editing && this.contains(this.getRootNode().activeElement)) {
                ...
                if (e.clipboardData.files.length > 0) {
                    ...
                    this.pasteData(html ? { 'text/plain': text, 'text/html': html } : text)
                }
            }
        })
    }
    ...
    paintSelection() {
        ...
        for (const row of this.mountedRows) {
            for (const column of this.columns) {
                ...
                // `readonly` is a per-binding property, applied in SpreadsheetCell#bind.
            }
        }
     ...
     * @returns {SpreadsheetCell|null} the focused cell, if currently mounted
     */
    focusCellAt(col, row) {
        this.active = { row, col }
        this.cursor = { row, col }
     ...
     *  - `session.commit({ move })` — end the edit, re-render the cell, optionally move the active cell
     *  - `session.cancel()` — end the edit without applying the bound input's change
     *  - `session.hold(promise)` — defer the closing re-render until the promise settles
     *    (e.g. an async save), however the edit ends (commit, blur, Enter/Tab)
     *
     ...
     */
    async activateCell(cell, session = {}) {
        ...
        // The editing session handed to the column's input (see the method JSDoc). Its
        // callbacks close over `floater`, which is assigned below — they can only run
        // once the editor exists, so the forward reference is safe.
        let floater
        Object.assign(session, {
            cell, column, record,
            pending: null,
            commit: opts => this.deactivateCell(opts), // arrow: keeps `this`
            hold: promise => { session.pending = promise; return promise },
            cancel: () => {
                floater.setAttribute('preventChange', true) // bound inputs check this before writing
                this.deactivateCell()
            }
        })

        // The session is the input's options bag too — `value` seeds type-to-edit. Columns
        // must pick what they need off it (not spread it into Input/element options): its
        // members would otherwise land as HTML attributes on the editor.
        const input = column.input(record, session)
        if (!input) return
        ...
        floater = new Floater({
            ...
            onHidden: async e => {
                ...
                // Hold the re-render for work the editor registered via session.hold (e.g.
                // an async save) so the cell reflects the settled value — this also covers
                // the blur-close path, which never goes through deactivateCell.
                if (session.pending) { try { await session.pending } catch { /* render whatever state remains */ } }
                // The awaited pending work may outlive the cell: windowing/sort/splice can
                // unmount and recycle it (nulling `column`) while the save is in flight.
                if (cell.column) cell.render() // reflect the value the bound input wrote (or the unchanged value on cancel)
                this.trigger('cellChanged', { detail: { cell, column, record } })
            }
        })
        this._editing = { cell, column, record, floater, session }
        cell.tabIndex = -1
        ...
        floater.addEventListener('keydown', e => {
            ...
            if (e.key === 'Enter' && !e.shiftKey) {
                requestAnimationFrame(() => {
                    e.preventDefault();
                    session.commit({move: 'down'})
                })
            }
            else if (e.key === 'Tab') {
                e.preventDefault();
                session.commit({move: e.shiftKey ? 'left' : 'right'})
            }
        ...
        floater.addEventListener('commit', e => {
            if (e.detail?.promise) session.hold(e.detail.promise)
            this.deactivateCell({ move: e.detail?.move })
        })

        floater.show() // appends into this.utilities (see container option above)

        // Focus the editor once it's positioned. A column may return a single bound element
        // or several nodes (e.g. an input plus an attribution badge/note), so query the
        // rendered editor: prefer the element the column marked `[autofocus]`, else the first
        // focusable control.
        requestAnimationFrame(() => {
            const editor = floater.querySelector('[autofocus]')
                || floater.querySelector(this.constructor.editorFocusableSelector)
            ...
            editor?.focus?.({ preventScroll: true })
            if (editor?.select && session.value == undefined) editor.select()
            if (editor?.showPicker && editor.tagName === 'SELECT') { /* selects open on focus/click */ }
     ...
     *
     * @param {Object} [options={}]
     * @param {string} [options.move] - direction for {@link Spreadsheet#moveActive} ('up'/'down'/'left'/'right'); omitted, focus returns to the cell
     */
    deactivateCell({ move } = {}) {
        if (!this._editing) return
        const { cell, floater } = this._editing
        floater.hide()
        if (move) this.moveActive(move)
    ...
    /** Escape text for inclusion in the `text/html` copy payload's `<td>` content. */
    escapeHtml(v) {
        return String(v).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]))
    }

    /**
     * Normalize a {@link DataGridColumn#valueFor} result into its `text/plain`/`text/html`
     * representations. A bare string (the common case) is `text/plain` only; a column may
     * instead return `{ 'text/plain', 'text/html' }` to carry a richer copy representation
     * (see {@link Spreadsheet#copyCells}).
     */
    valueRepresentations(value) {
        if (value != null && typeof value === 'object') {
            return { 'text/plain': value['text/plain'] ?? '', 'text/html': value['text/html'] ?? null }
        }
        return { 'text/plain': value == null ? '' : String(value), 'text/html': null }
    }
    ...
    async copyText() {
        ...
        for (let row = r.r0; row <= r.r1; row++) {
            ...
            for (let col = r.c0; col <= r.c1; col++) {
                ...
                const value = column.valueFor(await this.rows[row].resolvedRecordFor(column))
                cols.push(this.quote(this.valueRepresentations(value)['text/plain']))
            }
    ...
    /**
     * `<table>` markup for the selection, one `<tr>`/`<td>` per row/cell — the `text/html`
     * counterpart to {@link Spreadsheet#copyText}. Each `<td>` holds that cell's `text/html`
     * representation when its column's `valueFor` provided one, else its escaped plain value.
     */
    async copyHtml() {
        const r = this.range()
        if (!r) return ''
        ...
        for (let row = r.r0; row <= r.r1; row++) {
            ...
            for (let col = r.c0; col <= r.c1; col++) {
                const column = this.columns[col]
                const value = column.valueFor(await this.rows[row].resolvedRecordFor(column))
                const rep = this.valueRepresentations(value)
                cells.push(`<td>${rep['text/html'] ?? this.escapeHtml(rep['text/plain'])}</td>`)
            }
    ...
    copyCells() {
        ...
        // Show the marquee immediately; copyText/copyHtml are async (records may resolve), so
        // write the clipboard once they settle — via the async Clipboard API (can't use the
        // copy event's clipboardData after an await).
        this._copying = true
        this.paintSelection()
        Promise.all([this.copyText(), this.copyHtml()]).then(([tsv, html]) => {
            ...
            if (navigator.clipboard.write && typeof ClipboardItem !== 'undefined') {
                ...
                ]).catch(() => navigator.clipboard.writeText(tsv)) // e.g. permission denied for write([...])
            } else {
    ...
    /** Split TSV text into a matrix, dropping a trailing all-empty row left by a final newline. */
    matrixFromText(text) {
        const matrix = splitByUnquotedChar(text, "\n").map(r => splitByUnquotedChar(r, "\t"))
    ...
    /** Parse a pasted `text/html` payload's first `<table>` into a matrix of each `<td>`/`<th>`'s innerHTML, or `null` when it has no table. */
    matrixFromHtml(html) {
        const table = new DOMParser().parseFromString(html, 'text/html').querySelector('table')
        if (!table) return null
        const rows = [...table.querySelectorAll('tr')]
        if (!rows.length) return null
        return rows.map(tr => [...tr.querySelectorAll('td,th')].map(td => td.innerHTML))
    }

    /**
     * Build a paste matrix from `{ 'text/plain', 'text/html' }` clipboard data — the shape
     * read from a native `paste` event or {@link Spreadsheet#pasteFromClipboard}. When the
     * HTML carries a `<table>` shaped like the plain-text/TSV matrix (same row/column
     * counts), cells become `{ 'text/plain', 'text/html' }` pairs so {@link
     * Spreadsheet#pasteMatrixAt} can hand each column whichever representation it declared
     * support for. Otherwise (no table, or one that doesn't line up in row/column count —
     * e.g. external HTML with markup of its own) falls back to the plain-text matrix alone.
     */
    matrixFromClipboardData({ 'text/plain': text, 'text/html': html }) {
        const plainMatrix = text != null ? this.matrixFromText(text) : []
        const htmlMatrix = html ? this.matrixFromHtml(html) : null
        if (!htmlMatrix || htmlMatrix.length !== plainMatrix.length || htmlMatrix.some((row, i) => row.length !== plainMatrix[i].length)) {
            return plainMatrix
        }
        return htmlMatrix.map((row, r) => row.map((html, c) => ({ 'text/plain': plainMatrix[r][c], 'text/html': html })))
    }
    ...
    pasteData(data) {
        ...
        if (typeof data == 'string') {
            matrix = this.matrixFromText(data)
        } else if (data instanceof FileList) {
            matrix = [[data]]
        } else if (typeof data == 'object' && ('text/plain' in data || 'text/html' in data)) {
            matrix = this.matrixFromClipboardData(data)
        } else {
            matrix = [[data]]
        }
    ...
    pasteMatrixAt(matrix, row0, col0) {
        matrix.forEach((cols, dr) => cols.forEach(async (data, dc) => {
            ...
            if (typeof data == 'string') {
                ...
                // A `{ 'text/plain', 'text/html' }` pair (see matrixFromClipboardData) — hand
                // the column whichever representation it declared support for, most-preferred
                // first (column.pasteAccepts), falling back through the rest.
                const type = column.constructor.pasteAccepts.find(t => data[t] != null && column.accepts(t))
                if (type) {
                    await column.paste(record, data[type], type)
                } else {
                    this.trigger('invalidPaste', { detail: data })
                    return
                }
     ...
     * {@link Spreadsheet#setupSelection} handles ⌘V/Ctrl+V directly. When the async
     * Clipboard API or its permission is unavailable, fall back to {@link Spreadsheet#renderPasteModal}
    ...
    async pasteFromClipboard() {
        ...
        try {
            ...
            if (window.navigator.clipboard.read) {
                try {
                    for (const item of await window.navigator.clipboard.read()) {
                        if (item.types.includes('text/html')) {
                            html = await (await item.getType('text/html')).text()
                            break
                        }
                    }
                } catch { /* text/html unreadable (permission/unsupported) — fall back to plain text */ }
            }
            this.pasteData(html ? { 'text/plain': text, 'text/html': html } : text)
        } catch (e) {
            this.renderPasteModal()
