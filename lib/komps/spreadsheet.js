        ...
        fetch("/texas-rangers-roster.json").then(response => {
            response.json().then(data => {
                document.querySelector('div').append(new Spreadsheet({
                    ...
                    class: 'height-90-vh max-width-full',
                    data: data,
                    columns: [{
                            attribute: 'Name',
                        }, {
...
export default class Spreadsheet extends EditableDataGrid {
    ...
    renderCell (item, column, colIndex, rowIndex) {
        ...
        cell.render = () => {
            content(cell, result(column, 'render', item, column, cell))
        }
        cell.copy = () => column.copy(item)
        cell.paste = (v) => column.paste(item, v)
        if (column.input) {
            cell.input = () => this.renderInputCell(item, column, cell)
        }
    ...
    /* -------------------
        Cell Input
    ------------------- */
    async activateCell (cell, focusType='end') {
        if (cell.input === undefined || cell.input === null) { return }
        const inputCell = await cell.input()
        if (inputCell === undefined || inputCell === null) { return }
    ...
    async renderInputCell (item, column, originalCell) {
        if (column.input === null) { return }
        const content = await column.input(item, originalCell)
        if (content) {
            const cell = createElement(`${this.localName}-input`, {
                style: {
                    gridColumn: originalCell.column,
                    gridRow: originalCell.row
                },
                content: {
