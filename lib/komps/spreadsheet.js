...
- [x] Cell Selection
- [x] Copy/Paste
- [ ] scrollTo on focusCell when under frozenCell
- [ ] edit header
...
import { result, except, placeCaretAtEnd, closest, groupBy } from '../support';
    ...
    constructor (options, ...args) {
        ...
        this.addEventListenerFor(`${this.localName}-cell`, 'keydown', e => {
            if (["ArrowRight", "ArrowLeft", "ArrowDown", "ArrowUp"].includes(e.key)) {
                e.preventDefault()
                this.adjacentCell(e.delegateTarget, ...{
                    ArrowRight: [1, 0],
                    ArrowLeft: [-1, 0],
                    ArrowUp: [0, -1],
                    ArrowDown: [0, 1]
    ...
    activateCell (cell, focusType='end') {
        ...
        inputCell.addEventListener('keydown', e => {
            if (e.key == "Enter" && [e.metaKey,e.ctrlKey,e.altKey,e.shiftKey].every(x => x == false)) {
                const target = this.adjacentCell(cell, 0, 1)
