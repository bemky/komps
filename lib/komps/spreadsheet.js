...
export default class Spreadsheet extends DataTable {
    ...
    initializeAxisResize (e) {
        ...
        let dragOver = e => {
            ...
            this.trigger(this._columns[startIndex].id, `${axis}Resize`,  delta)
        }
    ...
    initializeCellSelection (startCell, e) {
        ...
        if (startCell.classList.contains('header')) {
            const colCells = this.querySelectorAll(`.col-${startCell.colIndex}`)
            colCells.forEach(cell => {
                cell.classList.add('selected')
            })
            this.highlightSelecedCells(colCells.item(colCells.length - 1), startCell)
