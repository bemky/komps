...
columns:
    types: Array
    description: define nested columns within a column, only used for generating header cells that this column will span
...
export default class DataTable extends KompElement {
    ...
    render () {
        ...
        this._columns.forEach(column => {
            const columns = Array.isArray(column.columns) ? column.columns.map(c => Object.assign({}, column, c)) : [column]
        ...
        this.data.forEach((item, rowIndex) => {
            ...
            this._columns.forEach((column, colIndex) => {
                ...
                if (column.columns) {
                    cell.style.gridColumn = `span ${column.columns.length}`
                    cell.style.display = 'grid'
                    cell.style.gridTemplateColumns = 'subgrid'
                }
                cell.render = () => {
                    content(cell, result(column, 'render', item, cell, column, this))
                }
                cell.colIndex = colIndex + 1
