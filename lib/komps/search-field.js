 ...
 * @param {Array|function} [options.defaultResults] - results shown when query is below `minLength` (including on focus before the user types). Same shape as items returned from `search`, so `result` and `select` apply unchanged. Function form receives no args and may return a Promise; resolved value is cached on the instance.
 * @param {string} [options.placeholder="Search"] - placeholder text for the input
 * @param {boolean|HTMLElement} [options.inline=false] - if true, renders results inline instead of in a floater. if HTMLElement, renders results in element
...
export default class SearchField extends KompElement {
    ...
    static assignableAttributes = {
        ...
        inline: { type: ['boolean', 'HTMLElement'], default: false, null: false },
        ...
        defaultResults: { type: ['object', 'function'], default: null, null: true }
    ...
    // @todo Breaking change: `detail` is a positional array, and its third
    // element is an ad-hoc source event. Normalize to
    // `{ result, query, sourceEvent }` to match the rest of the library
    // (Floater/Modal show/hide/remove, Dropzone#fileDrop). Deferred because it
    // breaks anyone destructuring the array — batch it with the next major.
    ...
    async _showDefaultResults () {
        if (!this.defaultResults) return;
        if (!this._defaultResultsCache) {
            const resolved = typeof this.defaultResults === 'function'
                ? await this.defaultResults()
                : this.defaultResults;
            this._defaultResultsCache = resolved || [];
        }
        this.results = this._defaultResultsCache;
        content(this.resultsList, this.renderResults(this.results));
        this.showResults();
    }
    ...
    showResults () {
        if (this.inline instanceof HTMLElement) {
            content(this.inline, this.resultsList)
        } else if (this.inline) {
            insertAfter(this.input, this.resultsList);
        } else {
            if (!this.floater) {
                this.floater = new Floater(Object.assign({
                    class: 'komp-search-field-floater',
