...
/**
 * Fired after the floater is shown. See {@link Floater#show} for `detail.sourceEvent`.
 * @event Floater#shown
 */
...
/**
 * Fired after the floater is hidden. See {@link Floater#hide} for `detail.sourceEvent`.
...
import { css, createElement, content } from 'dolla';

import KompElement from './element.js';

const HOLD_OPEN = 'hold-open';
...
export default class Floater extends KompElement {
    ...
    static bindMethods = ['show', 'hide', 'checkFocus', 'checkEscape', 'toggle', 'animationEnd']
    static middlewares = {
        size: size,
        shift: shift,
    ...
    connected () {
        ...
        this.bindAutoUpdate()
        this._cleanupCallbacks.push(() => this._autoUpdateCleanup?.())
        ...
        if (this.removeOnBlur) {
            ...
            this.cleanupEventListenerFor(this.getRootNode().body, 'keydown', this.checkEscape)
        }
    }
    ...
    bindAutoUpdate () {
        ...
        this._autoUpdateCleanup = () => {
            cleanup()
            delete this._autoUpdateCleanup
        }
    }
     ...
     */
    anchorChanged () {
        if (this.isConnected && this.anchor) { this.bindAutoUpdate() }
    }
     ...
     *
     * @param {string|HTMLElement|Array|Object} value - passed to {@link https://dollajs.com/#content Dolla's content}
    ...
    checkFocus (e) {
        if (this.hasAttribute(HOLD_OPEN)) return
        if (e.defaultPrevented) { return }
        if (e.target == this) { return }
        if (e.target == this.anchor) { return }
        if (this.contains(e.target)) { return }
        if (this.anchor.contains && this.anchor.contains(e.target)) { return }
        if (this.contains(this.getRootNode()?.activeElement)) { return }
        this.hide({detail: {sourceEvent: e}})
    }
    ...
    holdOpen () {
        this.setAttribute(HOLD_OPEN, '')
    }
    
    releaseHoldOpen () {
        this.removeAttribute(HOLD_OPEN)
    }
    
    async remove (eventOptions) {
        return new Promise(resolve => {
            this.classList.add('-out')
        
            const done = () => {
                this.classList.remove('-out')
                super.remove(eventOptions).then(resolve)
            }
    ...
    show (eventOptions) {
        ...
        if (this._removing) {
            // Don't pass this.show directly — the promise resolves with the
            // element, which would arrive as eventOptions.
            return this._removing.then(() => this.show(eventOptions))
        }
        ...
        if (!this.parentElement) {
            if (!this.trigger('show', eventOptions)) return
            this.showing = true

            this.activeFloater?.hideNow(eventOptions)
            this.activeFloater = this

            this.container.append(this)
            this.trigger('shown', eventOptions)
        }
        return this;
    }
    ...
    hideNow (eventOptions) {
        this.hideAfterTimeout(eventOptions)
    }
    ...
    hideAfterTimeout (eventOptions) {
        if (this.parentElement) {
            if (!this.trigger('hide', eventOptions)) return this
            return this._removing = this.remove(eventOptions).then(() => {
                this.showing = false
                this.trigger('hidden', eventOptions)
                delete this._hideTimeout;
     ...
     * @param {Object} [eventOptions] - CustomEvent options passed to the resulting show/hide events
     * @returns {Floater} this
     */
    toggle (shouldHide, eventOptions) {
        if (typeof shouldHide !== 'boolean') {
            shouldHide = this.offsetParent !== null
        }
        this[shouldHide ? 'hide' : 'show'](eventOptions)
        return this;
    }
