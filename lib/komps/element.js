...
import { content, remove, trigger, setAttributes, HTML_ATTRIBUTES } from 'dolla';
import { scanPrototypesFor, uniq, result } from '../support';
...
export default class KompElement extends HTMLElement {
    ...
    constructor(attrs={}) {
        super()
        this._assignableAttributes = uniq(scanPrototypesFor(this.constructor, 'assignableAttributes').filter(x => x).flat())
        this._assignableAttributes.forEach(attribute => {
            if (this.hasOwnProperty(attribute)){
                this._attributes[attribute] = this[attribute]
            }
    ...
    appendStyle () {
        if (this.constructor.style) {;
            ...
            if (root && root.adoptedStyleSheets && !root.adoptedStyleSheets.find(s => s.id != id)) {
                ...
                style.replaceSync(result(this.constructor, 'style'))
                style.id = id
