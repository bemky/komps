...
import { scanPrototypesFor, result, isFunction, eachPrototype } from '../support';
                        ...
                        if (!style) {
                            ...
                            if (p.tagName !== this.tagName) {
                                body = body.replaceAll(new RegExp(`${p.tagName}([^\,\{]*)`, 'g'), (match, suffix) => {
                                    return [p.tagName, this.tagName].map(x => x + suffix).join(", ")
                                })
                            }
                            ...
                            for (const rule of style.cssRules) {
                                rule.cssText = rule.cssText.replaceAll(p.tagName, [p.tagName, this.tagName].join(", "))
                            }
                        }
                    }
                })
