...
import { scanPrototypesFor, result, isFunction } from '../support';
...
export default class KompElement extends HTMLElement {
    ...
    instantiate () {
        ...
        return this.is_instantiated = true
    }
    ...
    connectedCallback () {
        ...
        if (!this.is_instantiated) {
            if (this.instantiate() === false) return
        }
        this.connected()
    ...
    /*
        Using root.styleSheets instead of root.adoptedStyleSheets because adopted stylesheets
        supercede all stylesheets in document.head
    */
    appendStyle () {
        if (this.constructor.style) {;
            const root = this.getRootNode()
            const id = this.tagName.toLowerCase()
            if (root && root.styleSheets) {
                for (const style in root.styleSheets) {
                    if (style.id == id) {
                        return style
                    }
                }
                ...
                let markup = ""
                scanPrototypesFor(this.constructor, 'style').forEach(style => {
                    markup += isFunction(style) ? style.bind(this.constructor)() : style
                })
    ...
    manageEventListenerFor(...listener) {
