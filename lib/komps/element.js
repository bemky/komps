...
export default class KompElement extends HTMLElement {
        ...
        scanPrototypesFor(this.constructor, 'events').filter(x => x).reverse().forEach(events => {
            events.forEach(event => {
                ...
                if (attrs.hasOwnProperty(eventKey)) {
                    const fn = attrs[eventKey]
                    if (typeof fn === 'function') {
                        if (fn.length > 1) {
                            console.warn(`[Komps] "${event}" listener appears to use the old callback signature. It now receives a single Event object with data on event.detail.`)
                        } else if (fn.length === 1) {
                            const firstArg = fn.toString().match(/^(?:function\s*)?\(?\s*([^,)=\s]+)/)?.[1]
                            if (firstArg && firstArg !== 'e' && firstArg !== 'event') {
                                console.warn(`[Komps] "${event}" listener appears to use the old callback signature. It now receives a single Event object with data on event.detail.`)
                            }
                        }
                    }
     ...
     * @param {Object} [eventOptions] - CustomEvent options for the remove and removed events. Data belongs on `detail`.
     * @returns {KompElement} this
     */
    async remove (eventOptions) {
        let callback
        if (isFunction(eventOptions)) {
            console.warn("[Komps] remove(callback) is deprecated, instead pass event options: remove(eventOptions).")
            callback = eventOptions
            eventOptions = undefined
        }
        if (!this.trigger('remove', eventOptions)) return this
        if (callback) await callback()
        super.remove()
        this.trigger('removed', eventOptions)
        return this;
    }
