...
export default class Modal extends KompElement {
    ...
    connected () {
        ...
        const heading = this.querySelector('h1, h2, h3, h4, h5, h6, [id]');
        if (heading) {
            if (!heading.id) heading.id = `${this.localName}-title-${Math.random().toString(36).slice(2, 9)}`;
        ...
        if (this.parentElement.localName != `${this.localName}-popover`) {
            ...
            this.cleanupEventListenerFor(this.container, 'keydown', e => {
                if (e.key === 'Escape') {
                    e.preventDefault()
                    this.remove({detail: {sourceEvent: e}})
                    return
                }
                if (e.key === 'Tab') {
            ...
            const initialFocus = this.container.querySelector('[autofocus]') || this.container.querySelector(Modal.FOCUSABLE_SELECTOR)
            if (initialFocus) {
                initialFocus.focus()
            }
        }
    ...
    static style = function() { return `
        ...
        ${this.tagName}-close button:focus {
            opacity: 1;
            color: blue;
        }
    `}
