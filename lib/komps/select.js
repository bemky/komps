...
export default class Select extends FormField {
    ...
    onKeyDown (e) {
        if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
            ...
            e.preventDefault()
            if (!this.dropdown.showing) this.dropdown.show({detail: {sourceEvent: e}})
            if (e.key === 'ArrowUp') {
            ...
            this.typeahead(e.key, {detail: {sourceEvent: e}})
        }
    }

    typeahead (char, eventOptions) {
        if (!this.dropdown) {
        ...
        if (match) {
            if (!this.dropdown.showing) this.dropdown.show(eventOptions)
            match.focus()
        }
    }
