...
export default class Tooltip extends Floater {
     ...
     * middleware built in `initialize()` — instead of building a new one.
     *
     * A connected tooltip is detached first so `connected()` runs again on the
     * next `show()`, which rebinds the `autoUpdate` loop to the new anchor.
     * That detach is a re-target rather than a dismissal, so it deliberately
     * skips the exit animation and the hide/hidden events.
     *
