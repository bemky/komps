 ...
 *
 * @fires Resizer#resize - cancellable; call `preventDefault()` to skip default resize-to-target behavior
 *
 * @example
...
export default class Resizer extends KompElement {
    ...
    activate (e) {
        ...
        window.addEventListener('mouseup', () => {
            const event = new CustomEvent('resize', { cancelable: true, detail: { resizer: this } })
    ...
    /**
     * Default resize behavior: syncs position/size back to the target
     * element's inline styles. Runs after a resize/move completes unless
     * the `resize` event is cancelled via `preventDefault()`.
     */
    applyResize () {
