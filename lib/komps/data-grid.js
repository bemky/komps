...
/** Resolve a column's `width` config (px number or px string) to a number, or `fallback`. */
...
export default class DataGrid extends KompElement {
    ...
    // Surface mirrored from Table/Spreadsheet so a future `Spreadsheet extends DataGrid`
    // can inherit these (see plan §3). Not all are emitted in the read-only v1.
    static events = [
        'headerChange', 'headerChanged',
        'indexChange', 'indexChanged',
        'widthChange', 'widthChanged',
        'columnRemoved', 'columnsChanged',
        'rendered'
    ]
    ...
    async initialize(...args) {
        ...
        this.columnGeometry = new DataGridGeometry(this.columns, {
            sizeOf: col => resolveWidth(col.width, this.defaultColumnWidth)
        })
    ...
    setupObservers() {
        this.addEventListener('scroll', () => this.scheduleWindow())
        this._viewportObserver = new ResizeObserver(() => this.scheduleWindow())
    ...
    syncMounted(start, end) {
        // Unmount rows that fell out of range.
        for (const row of Array.from(this.mounted)) {
            if (row.index < start || row.index > end) {
                if (this._rowObserver) this._rowObserver.unobserve(row.element)
                row.unmount()
                this.mounted.delete(row)
            }
        }
        // Mount rows newly in range.
        for (let i = start; i <= end; i++) {
            const row = this.rows[i]
            if (row && !row.mounted) {
                row.mount()
                if (this._rowObserver) this._rowObserver.observe(row.element)
                this.mounted.add(row)
            }
        }
    }
    ...
    releaseRowElement(el) {
        el.remove() // detach from the body so unmounted rows don't pile up in the DOM
        el.replaceChildren()
    ...
    ----------------------------------------------------------------- */
    /** All row controllers (each serves as the row handle). */
    get rowControllers() { return this.rows }
    ...
    queryCell(selector = '') { return this.queryCells(selector)[0] || null }
