 ...
 * @param {function} [options.loadRecords] - `(records) => Promise` batch loader called with the records of rows entering the window (overscan included), before their cells render. Hydrate the records in place (e.g. fetch and merge nested associations) and resolve when done; cells render once the batch settles. Mount-driven only: records accessed while off-screen (e.g. Spreadsheet copy across an off-screen range) are not loaded.
 * @param {function} [options.loadingRow] - `(record, row) => content` rendered inside a row while `loadRecords` is resolving its record; replaced by cells once the load settles
 *
...
export default class DataGrid extends KompElement {
    ...
    static assignableAttributes = {
        ...
        loadingRow: { type: 'function', default: null, null: true },
        loadRecords: { type: 'function', default: null, null: true }
    }
    ...
    async initialize(...args) {
        ...
        this.columnGeometry = new DataGridGeometry(this.columns, {
            // A column's geometry size is its total width across all its tracks, so
            // offsets/extent stay one-entry-per-column even with colspan sub-tracks.
            sizeOf: col => col.totalWidth(this.defaultColumnWidth)
        })
        this.renderScaffold()
        this.setupObservers()
     ...
     */
    applyColumnGeometry() {
        this.assignColumnLines()
        this.style.setProperty('--dg-template-columns', this.columnTemplate())
        this.style.setProperty('--dg-width', this.columnGeometry.extent + 'px')
    }

    /**
     * Assign each column its 1-based grid start line — a prefix-sum of colspans, so a
     * `colspan>1` column pushes the ones after it along by that many tracks. Recomputed
     * on every geometry apply (initial render, splice, resize, reorder).
     */
    assignColumnLines() {
        let line = 1
        for (const column of this.columns) {
            column.startLine = line
            line += (column.colspan || 1)
        }
    }

    /**
     * The expanded `grid-template-columns` track list: each column contributes its
     * `colspan` sub-tracks (a single width track for the common colspan-1 case). Total
     * track count is the sum of colspans; total width still equals the geometry extent.
     */
    columnTemplate() {
        return this.columns
            .flatMap(column => column.trackSizes(this.defaultColumnWidth))
            .map(px => px + 'px')
            .join(' ')
    }
    ...
    renderHeader() {
        ...
        this.columns.forEach(column => {
            ...
            if ((column.colspan || 1) > 1) {
                // Subgrid so header content can place onto the column's sub-tracks,
                // lining up with the colspan cells below (see DataGridColumn#colspan).
                cell.style.display = 'grid'
                cell.style.gridTemplateColumns = 'subgrid'
    ...
    syncMounted(start, end) {
        ...
        for (let i = start; i <= end; i++) {
            ...
            if (row && !row.mounted) rowsToMount.push(row)
        }
        if (this.loadRecords) {
            // Batch-load the records of rows entering the window (one call per window,
            // overscan included). Rows mount immediately and render `loadingRow` content
            // until the batch settles (see DataGridRow#renderCells).
            const rowsToLoad = rowsToMount.filter(row => !row.loading)
            if (rowsToLoad.length > 0) {
                // Settle (not just resolve) so a failed load still renders rows with
                // their unloaded records instead of wedging the placeholders.
                const load = Promise.resolve(this.loadRecords(rowsToLoad.map(r => r.record)))
                    .catch(error => console.error('[Komps] DataGrid loadRecords failed; rendering rows with unloaded records.', error))
                for (const row of rowsToLoad) row.loading = load
            }
        }
        for (const row of rowsToMount) row.mount()
    }
    ...
    static style = function () { return `
        ...
        /* The cell's content block (see DataGridCell#contentElement) takes no part in
           layout until something needs to bound the content, so a cell renders exactly
           as it would without it — including cells the app formats as a flex or grid
           container, whose children would otherwise be laid out inside this box.
           The collapsible plugin gives it a box for the cells it truncates. */
        ${this.tagName}-cell-content {
            display: contents;
        }
