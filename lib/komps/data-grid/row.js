...
export default class DataGridRow {
    ...
    /** Detach: return cells + row element to their pools and drop references. */
    unmount() {
        if (!this.element) return
        if (this.cellsByColumn) {
            this.cellsByColumn.forEach(cell => {
                if (cell.column) cell.column.cells.delete(cell)
                this.grid.releaseCell(cell)
            })
            this.cellsByColumn = null
        }
        this.element.row = null
        this.grid.releaseRowElement(this.element)
        this.element = null
    }
}
