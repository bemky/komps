...
export default class EditableDataGrid extends DataGrid {
    ...
    renderHeaderCell (column, index) {
        ...
        if (this.reorder.includes('columns')) {
            cell.classList.add('reorderable')
    ...
    renderCell (record, column, colIndex, rowIndex) {
        ...
        if (this.resize.includes('rows')) {
            cell.classList.add('resizable-row')
            cell.classList.toggle('resizable', colIndex == 1)
        }
        if (this.resize.includes('columns')) {
            cell.classList.toggle('resizable-column', !!column.resize)
        }
        ...
        if (this.reorder.includes('rows')) {
            ...
            cell.classList.toggle('reorderable', colIndex == 1)
        }
        if (this.reorder.includes('columns')) {
            cell.classList.toggle('reorderable-column', !!column.reorder)
        }
    ...
    activateAxisReorder (e) {
        ...
        const dragOver = e => {
            e.preventDefault()
            const cell = closest(e.target, `${this.tagName}-cell.reorderable-${direction.toLowerCase()}, ${this.tagName}-reorder-handle`) || previousCell
            previousCell = cell;
