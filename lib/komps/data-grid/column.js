 ...
 * @param {number} [options.colspan=1] - number of grid tracks this column occupies. `>1` splits the column into sub-tracks a cell can address (e.g. via `grid-template-columns: subgrid`) so its content aligns into aligned sub-columns across every row. Sub-tracks must be fixed px summing to `width` — see `tracks` — so the px-based column geometry (offsets, selection, resize) stays in sync
 * @param {number[]|string} [options.tracks] - explicit sub-track sizes for a `colspan>1` column: a px-number array or a space-separated px string (`[120,120,120]` / `"120px 120px 120px"`). Their sum is the column's total width. Omit to split `width` evenly across `colspan` tracks
...
export default class DataGridColumn {
     ...
     * `colspan` tracks that sum to the column's width. Explicit `tracks` win;
    ...
    trackSizes(defaultWidth) {
        ...
        if (this.tracks != null) {
            const list = Array.isArray(this.tracks) ? this.tracks : String(this.tracks).trim().split(/\s+/)
     ...
     * clamped to `min`. Materializes explicit `tracks` from the current sizes first, so
     * it works on default-split columns too. No-op without a right neighbor.
     */
    resizeTrack(i, size, min = 0, defaultWidth) {
        const tracks = this.trackSizes(defaultWidth)
        if (i < 0 || i + 1 >= tracks.length) return
        const pair = tracks[i] + tracks[i + 1]
        tracks[i] = Math.max(min, Math.min(size, pair - min))
        tracks[i + 1] = pair - tracks[i]
        this.tracks = tracks
    }
