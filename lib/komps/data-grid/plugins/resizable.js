...
export default function (proto) {
    ...
    /* -----------------------------------------------------------------
       Hover grips — transient, one per axis at a time, revealed on the axis's
       *first cell* (header cell for a column, leading cell for a row) but
       rendered into the grid's overlay layer so they aren't clipped by the
       cells' `overflow: hidden` and can straddle the dividing edge. Positioned
       from the live cell rect (which already reflects frozen/sticky offsets);
       clear-on-scroll keeps them aligned.
    ----------------------------------------------------------------- */
    ...
    /**
     * Place `handle` on `cell`'s dividing edge in overlay (scroll-content) space:
     * a column grip on the trailing edge spanning the cell's height, a row grip on
     * the bottom edge spanning the cell's width.
     */
    proto.positionHandle = function (handle, cell, axis) {
    ...
    proto.showColumnHandle = function (headerCell) {
        ...
        const handle = this.makeHandle('column', column.index, (e, i) => this.startColumnResize(e, i))
        this.positionHandle(handle, headerCell, 'column')
        this.resizeLayer().appendChild(handle)
    ...
    proto.showRowHandle = function (cell) {
        ...
        this.clearResizeHandles()
        const handle = this.makeHandle('row', cell.rowIndex, (e, i) => this.startRowResize(e, i))
        this.positionHandle(handle, cell, 'row')
        this.resizeLayer().appendChild(handle)
