/**
 * A plugin to make a {@link DataGrid} (and its subclasses, e.g. {@link Spreadsheet})
...
export default function (proto) {
    ...
    // plugin included on a subclass (Spreadsheet) doesn't leak `resize` onto DataGrid.
    ...
    this.events.push('columnResized', 'rowResized', 'subColumnResized')
    ...
    proto.setupResizeHandles = function () {
        if (this.resize.includes('columns')) {
            this.addEventListenerFor(`${this.localName}-header-cell`, 'mouseover', e => this.showColumnResizeHandle(e.delegateTarget))
        }
        if (this.resize.includes('rows')) {
            this.addEventListenerFor(`${this.localName}-cell`, 'mouseover', e => this.showResizeRowHandle(e.delegateTarget))
        }
    ...
    proto.addResizeEdgeHandle = function (layer, cell, axis, edge, targetIndex) {
        ...
        this.positionResizeHandle(handle, cell, axis, edge)
        layer.appendChild(handle)
        return handle
    }

    proto.positionResizeHandle = function (handle, cell, axis, edge) {
    ...
    proto.showColumnResizeHandle = function (headerCell) {
        ...
        this.addResizeEdgeHandle(layer, headerCell, 'column', 'end', column.index)
        const prev = this.columns[column.index - 1]
        if (prev && prev.resize !== false) this.addResizeEdgeHandle(layer, headerCell, 'column', 'start', prev.index)
        // Colspan columns: a grip at each internal sub-track divider, redistributing
        // width between the two sub-columns it splits (the column total is unchanged).
        if ((column.colspan || 1) > 1) this.addSubColumnHandles(layer, headerCell, column)
        this._columnHandleIndex = column.index
    }

    /**
     * Dotted grips over the header at each internal sub-track boundary of a colspan
     * column. Positioned from the header cell's left edge plus the cumulative sub-track
     * widths, so they sit on the real dividers regardless of column offset/scroll.
     */
    proto.addSubColumnHandles = function (layer, headerCell, column) {
        const tracks = column.trackSizes(this.defaultColumnWidth)
        const grid = this.getBoundingClientRect()
        ...
        for (let j = 0; j < tracks.length - 1; j++) {
            acc += tracks[j]
            const handle = createElement(`${this.localName}-resize-handle`, { class: 'column sub', content: { tag: 'grip' } })
            handle.addEventListener('mousedown', e => { e.stopPropagation(); e.preventDefault() })
            handle.addEventListener('pointerdown', e => {
                e.stopPropagation()
                this.startSubColumnResize(e, column, j, headerCell)
            })
            handle.style.left = (r.left + acc - ox) + 'px'
            handle.style.top = (r.top - oy) + 'px'
            handle.style.height = r.height + 'px'
            layer.appendChild(handle)
        }
    ...
    proto.showResizeRowHandle = function (cell) {
        ...
        this.clearResizeHandles()
        const layer = this.utilities
        this.addResizeEdgeHandle(layer, cell, 'row', 'end', cell.rowIndex)
        if (cell.rowIndex > 0) this.addResizeEdgeHandle(layer, cell, 'row', 'start', cell.rowIndex - 1)
    ...
    proto.startSubColumnResize = function (e, column, j, cell) {
        if (e.button != null && e.button !== 0) return
        const tracks = column.trackSizes(this.defaultColumnWidth)
        // Base = left edge of sub-track j (in body space); size grows/shrinks it,
        // bounded so both it and its right neighbor keep at least resizeMin.
        const base = this.columnGeometry.offsetAt(column.index) + tracks.slice(0, j).reduce((s, t) => s + t, 0)
        this.beginResize({
            axis: 'column',
            cell,
            pointerId: e.pointerId,
            ...
            base,
            min: this.resizeMin,
            max: tracks[j] + tracks[j + 1] - this.resizeMin,
            commit: size => this.resizeSubColumn(column, j, size)
        })
    }
    ...
    proto.beginResize = function ({ axis, index, cell, pointerId, startPos, clientProp, startSize, commit, base: baseOverride, min: minOverride, max }) {
        const isCol = axis === 'column'
        const geom = isCol ? this.columnGeometry : this.rowGeometry
        // Sub-track dividers pass their own base (the sub-track's left edge) and bounds.
        const base = baseOverride != null ? baseOverride : geom.offsetAt(index) + (isCol ? 0 : this.body.offsetTop)
        const min = minOverride != null ? minOverride : this.resizeMin
        ...
        this.clearResizeHandles()
        this.clearReorderHandles?.() // if the reorder plugin is also on, drop its grips too
        this.resizing = true
        ...
        const move = ev => {
            ...
            if (max != null) size = Math.min(size, max)
        ...
        const finish = ev => {
            ...
            setTimeout(() => {
                ...
                if (cell) (isCol ? this.showColumnResizeHandle(cell) : this.showResizeRowHandle(cell))
            })
        }
    ...
    proto.resizeColumn = function (index, width) {
        ...
        if (!column) return
        column.setWidth(width) // rescales colspan sub-tracks so their ratios hold
        ...
        this.sizeBody()
        this.paintSelection?.()      // Spreadsheet: keep the range box aligned to the new geometry
        this.trigger('columnResized', { detail: { column, width } })
    }

    proto.resizeSubColumn = function (column, j, size) {
        column.resizeTrack(j, size, this.resizeMin, this.defaultColumnWidth)
        this.columnGeometry.rebuildFrom(column.index) // total unchanged; refresh sizes cache
        this.applyColumnGeometry()                    // re-publish the expanded sub-track template
        this.paintSelection?.()
        this.trigger('subColumnResized', { detail: { column, index: j, tracks: column.trackSizes(this.defaultColumnWidth) } })
    }
    ...
    this.style.push(function () { return `
        ${this.tagName} {
            --resize-handle-color: hsla(0, 0%, 0%, 0.2);
            --select-color: #1a73e8;
        ...
        /* Sub-track divider: a dotted line spanning the header height, with a small
           handle knob, distinct from the solid whole-column grips at the edges. */
        ${this.tagName}-resize-handle.column.sub > grip {
            border-inline-width: 0;
            border-inline-start: 1px dotted var(--resize-handle-color);
            inline-size: 0;
            block-size: 100%;
            max-block-size: none;
        }
        ${this.tagName}-resize-handle.column.sub > grip::after {
            content: '';
            position: absolute;
        ...
        ${this.tagName}-resize-handle.column.sub:hover > grip,
        ${this.tagName}-resize-handle.column.sub:hover > grip::after {
            border-inline-start-color: var(--select-color);
