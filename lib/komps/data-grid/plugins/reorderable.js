 ...
 *
 * The **virtualized counterpart** to `table/plugins/reorderable.js`: it shares the UI (a
 * hover grip, a translucent drag ghost, a coloured placement line, the `grab`/`grabbing`
 * cursors and `--reorder-color` accent) but is rewritten against the grid's model rather
 * than the DOM. Off-screen cells don't exist, so the drop target is resolved from the
 * row/column **geometry** (a hit-test over cumulative offsets) and the reorder mutates the
 * `columns` / `rows` **controller arrays**, then re-publishes geometry once.
 *
 * **Cheap drag.** Only the ghost + placement line move while dragging — the model is left
 * alone until `pointerup`, where a single splice/re-index/geometry-rebuild lands the move.
 * Columns re-place via `grid-column` + `--dg-template-columns` (no DOM reshuffle); rows
 * move their controller and re-window. Auto-scrolls when the pointer nears an edge.
 *
...
function normalizeReorder(reorder) {
    if (reorder === true) return ['columns', 'rows']
    if (reorder === false || reorder == null) return []
    if (typeof reorder === 'string') return [reorder]
    return reorder
}
...
export default function (proto) {
    ...
    proto.initialize = async function (...args) {
        this.reorder = normalizeReorder(this.reorder)
    ...
    /** Re-place every header + mounted cell on its column's new track (no DOM reshuffle). */
