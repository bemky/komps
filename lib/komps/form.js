 ...
 * @param {function} [options.beforeSubmit] - event listener for beforeSubmit. Call `event.preventDefault()` to cancel.
 * @param {function} [options.onBeforeSubmit] - alias for beforeSubmit event listener
 * @param {function} [options.afterSubmit] - event listener fired after successful submit
 * @param {function} [options.onAfterSubmit] - alias for afterSubmit event listener
 *
...
export default class Form {
    ...
    constructor (target, options = {}) {
        ...
        if (beforeSubmit || onBeforeSubmit) {
            this.el.addEventListener('beforeSubmit', (beforeSubmit || onBeforeSubmit))
        }
        if (afterSubmit || onAfterSubmit) {
            this.el.addEventListener('afterSubmit', (afterSubmit || onAfterSubmit))
        }
    ...
    async onSubmit(e) {
        ...
        const beforeEvent = new CustomEvent('beforeSubmit', { cancelable: true, detail: { target: this.target } })
        ...
        try {
            ...
            this.el.dispatchEvent(new CustomEvent('afterSubmit', { detail: { target: this.target } }))
        } finally {
            this.enableButtons()
        }
    }
