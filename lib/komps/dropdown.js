...
export default class Dropdown extends Floater {
    ...
    static bindMethods = ['anchorMouseevent', 'anchorKeydown', 'keyDown', '_clearHide', 'hide']
    ...
    connected (...args) {
        ...
        this.addEventListener('mouseenter', this._clearHide)
    ...
    disconnected () {
        ...
        this.removeEventListener('mouseenter', this._clearHide)
        this.removeEventListener('mouseleave', this.hide)
    }
    
    keyDown (e) {
        if (e.key === 'Tab') {
            // Defer until after the browser's native Tab focus-move completes,
            // then only close if focus actually landed outside the dropdown —
            // tabbing between fields inside it (e.g. a filter panel with
            // several inputs) shouldn't close it.
            setTimeout(() => {
     ...
     * @param {Object} [eventOptions] - CustomEvent options passed to the show/shown events
     * @returns {Dropdown} this
     */
    show (eventOptions) {
        super.show(eventOptions)
     ...
     * @param {Object} [eventOptions] - CustomEvent options passed to the hide/hidden events
     * @returns {Dropdown} this
     */
    hide (eventOptions) {
        super.hide(eventOptions)
    ...
    anchorChanged (was, now) {
        ...
        if (this.mouseevent == "mouseenter") {
            ...
            if (was && was instanceof HTMLElement) {
                was.removeEventListener(this.mouseevent, this.anchorMouseevent)
                was.removeEventListener('keydown', this.anchorKeydown)
            }
            if (now && now instanceof HTMLElement) {
                now.addEventListener(this.mouseevent, this.anchorMouseevent)
    ...
    anchorMouseevent (e) {
        this.toggle(undefined, {detail: {sourceEvent: e}})
    }
